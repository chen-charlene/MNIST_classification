...
class Dense(Diffable):
    ...
    def call(self, x: Tensor) -> Tensor:
        ...
        """
        dot_product = np.matmul(x,self.w)
    ...
    def get_weight_gradients(self) -> list[Tensor]:
        ...
        """
        grad_weight = np.ones_like(self.w) * np.expand_dims(self.inputs[0], axis=-1)
        grad_bias = np.ones_like(self.b)
    ...
    def _initialize_weight(initializer, input_size, output_size) -> tuple[Variable, Variable]:
        ...
        if initializer == "normal":
            weights = Variable(np.random.normal(0,1,io_size))
        elif initializer == "xavier":
            stdev = sqrt(2/(input_size + output_size))
            weights = Variable(np.random.normal(0,stdev,io_size))
        elif initializer == "kaiming":
            stdev = sqrt(2/input_size)
            weights = Variable(np.random.normal(0,stdev,io_size))
        else:
